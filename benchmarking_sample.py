...
# Prepare table with A-domains and their original amino acids
ground_truth_data = pd.read_csv('bench_full_platinum.tsv', sep='\t')
filtered_adoms = ground_truth_data[ground_truth_data['aSDomain'].str.contains('AMP-binding')]
...
selected_columns_puma = ['id_y', 'locus_tag', 'module', 'substrate']
origin_puma = filtered_adoms[selected_columns_puma].copy()
origin_puma['substrate'] = origin_puma['substrate'].apply(lambda x: replace_characters(x))
origin_puma.rename(columns={'id_y': 'id', 'substrate': 'true'}, inplace=True)
origin_puma['id'] = origin_puma['id'].str.split('.').str[0]
...
df2 = origin_puma.merge(prediction_puma, on=['id', 'locus_tag', 'module'])
